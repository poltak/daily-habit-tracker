...
export function SetupView({ data, onRefresh, onMessage, onBusyChange }: SetupViewProps) {
  ...
  function goalActivityOptions(goal: Goal) {
    const linked = effectiveActivities.find((activity) => activity.id === goal.activityId);
      ...
      <section className="page-section settings-page">
        ...
        <section className="settings-card">
            ...
            {effectiveGoals.map((goal, index, all) => {
              ...
              return <div className={`management-row ${archived ? "archived" : ""}`} key={goal.id}>
                ...
                <span className="management-actions">
                  ...
                  <select className={`tiny-select ${pending ? "pending-action" : ""}`} aria-label={`Change activity for ${goal.name}`} aria-busy={pending} disabled={pending} value={goal.activityId} onChange={(event) => void patchCatalog({ kind: "goal", id: goal.id, patch: { activityId: event.target.value }, optimistic: { activityId: event.target.value } })}>{goalActivityOptions(goal).map((activity) => <option key={activity.id} value={activity.id}>{activity.name}{activity.archived ? " (archived)" : ""}</option>)}</select>
          ...
          <div className="inline-form stacked-mobile">
            ...
            <select value={goalActivity} onChange={(event) => setGoalActivity(event.target.value)} aria-busy={isKindReordering("goal") || isPending("create:goal")} disabled={isKindReordering("goal") || isPending("create:goal")}>{activeActivities.map((activity) => <option key={activity.id} value={activity.id}>{activity.name}</option>)}</select>
            <select value={goalSchedule} onChange={(event) => setGoalSchedule(event.target.value as Goal["scheduleType"])} aria-busy={isKindReordering("goal") || isPending("create:goal")} disabled={isKindReordering("goal") || isPending("create:goal")}><option value="daily">Every day</option><option value="weekdays">Selected weekdays</option><option value="times_per_week">Several times a week</option></select>
            <button className={`secondary-button ${isKindReordering("goal") || isPending("create:goal") ? "pending-action" : ""}`} aria-busy={isKindReordering("goal") || isPending("create:goal")} disabled={isKindReordering("goal") || isPending("create:goal")} onClick={() => void create({ payload: { kind: "goal", name: goalName, activityId: goalActivity, scheduleType: goalSchedule }, reset: () => setGoalName("") })}><Icon name={isPending("create:goal") ? UI_ICONS.sync : UI_ICONS.add} /> {isPending("create:goal") ? "Adding…" : isKindReordering("goal") ? "Reordering…" : "Add goal"}</button>
          </div>
        </section>
