...
function SettingsView({ data, onRefresh, onMessage }: { data: Bootstrap; onRefresh: () => Promise<void>; onMessage: (message: { kind: "success" | "error"; text: string }) => void }) {
  ...
  async function setIcon(id: string, current: string) {
    const icon = window.prompt("Material Symbols Rounded name (for example: menu_book)", current)?.trim();
    if (icon && icon !== current) await patchCatalog("activity", id, { icon });
  }
