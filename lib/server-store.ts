...
export class D1DaylioStore {
  ...
  async setActivitySelection(logicalDate: string, activityId: string, selected: boolean): Promise<SelectionMutationResult> {
    ...
    for (const goal of linkedGoals) {
      if (selected) {
        statements.push(this.database.prepare("INSERT INTO goal_completions (id, goal_id, logical_date, entry_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(goal_id, logical_date) DO UPDATE SET entry_id = excluded.entry_id, updated_at = excluded.updated_at").bind(`completion-${goal.id}-${logicalDate}`, goal.id, logicalDate, entry?.id ?? null, timestamp, timestamp));
      } else {
        statements.push(this.database.prepare("DELETE FROM goal_completions WHERE goal_id = ? AND logical_date = ?").bind(goal.id, logicalDate));
      }
    }
    await this.database.batch(statements);
    const selection = { logicalDate, activityId, selected };
    return {
      selection,
      affectedGoalCompletions: linkedGoals.map((goal) => ({ goalId: goal.id, logicalDate, completed: selected, entryId: entry?.id })),
  ...
  async setGoalCompletion(logicalDate: string, goalId: string, completed: boolean): Promise<SelectionMutationResult> {
    ...
    const statements = linkedGoals.map((linkedGoal) => completed
      ...
      : this.database.prepare("DELETE FROM goal_completions WHERE goal_id = ? AND logical_date = ?").bind(linkedGoal.id, logicalDate));
    if (goal.activity_id) statements.push(this.database.prepare("INSERT INTO day_activity_selections (logical_date, activity_id, selected, created_at, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(logical_date, activity_id) DO UPDATE SET selected = excluded.selected, updated_at = excluded.updated_at").bind(logicalDate, goal.activity_id, completed ? 1 : 0, timestamp, timestamp));
    await this.database.batch(statements);
    const selection = goal.activity_id ? { logicalDate, activityId: goal.activity_id, selected: completed } : undefined;
    return {
      completion: { goalId, logicalDate, completed, entryId: entry?.id },
      selection,
      affectedGoalCompletions: linkedGoals.map((linkedGoal) => ({ goalId: linkedGoal.id, logicalDate, completed, entryId: entry?.id })),
  ...
  private async applyImport(payload: ImportPayload) {
    ...
    const allStatements = [
      ...payload.moods.map((item) => {
        const moodId = moodIds.get(item.sourceId);
        const canonicalMood = MOODS.find((mood) => mood.id === moodId || mood.score === item.score || mood.name.toLowerCase() === item.name.toLowerCase());
        return this.database.prepare("INSERT INTO mood_levels (id, score, name, emoji, color, sort_order, source_system, source_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, score=excluded.score, emoji=excluded.emoji, color=excluded.color, source_system=excluded.source_system, source_id=excluded.source_id").bind(moodId, canonicalMood?.score ?? item.score, canonicalMood?.name ?? item.name, canonicalMood?.emoji ?? item.emoji ?? "🙂", canonicalMood?.color ?? "#9aa4ae", (canonicalMood?.score ?? item.score) * -1, "daylio", item.sourceId);
      }),
      ...payload.groups.map((item) => this.database.prepare("INSERT INTO activity_groups (id, name, sort_order, archived_at, source_system, source_id) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, sort_order=excluded.sort_order, archived_at=excluded.archived_at").bind(groupIds.get(item.sourceId), item.name, item.sortOrder, item.archived ? importedAt : null, "daylio", item.sourceId)),
      ...payload.activities.map((item) => this.database.prepare("INSERT INTO activities (id, group_id, name, material_icon, source_icon_id, source_state, sort_order, archived_at, source_system, source_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, group_id=excluded.group_id, material_icon=excluded.material_icon, source_icon_id=excluded.source_icon_id, source_state=excluded.source_state, sort_order=excluded.sort_order, archived_at=excluded.archived_at").bind(activityIds.get(item.sourceId), groupIds.get(item.groupSourceId ?? ""), item.name, iconForActivity(item.name, item.sourceIconId), item.sourceIconId ?? null, item.sourceState ?? null, item.sortOrder, item.archived ? importedAt : null, "daylio", item.sourceId)),
      ...payload.goals.map((item) => {
        ...
        return this.database.prepare("INSERT INTO goals (id, activity_id, name, material_icon, repeat_type, schedule_type, target_per_week, weekdays_mask, sort_order, reminder_enabled, reminder_time, archived_at, source_system, source_id, source_state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, activity_id=excluded.activity_id, material_icon=excluded.material_icon, repeat_type=excluded.repeat_type, schedule_type=excluded.schedule_type, target_per_week=excluded.target_per_week, weekdays_mask=excluded.weekdays_mask, source_state=excluded.source_state, sort_order=excluded.sort_order, reminder_enabled=excluded.reminder_enabled, reminder_time=excluded.reminder_time, archived_at=excluded.archived_at").bind(goalIds.get(item.sourceId), item.activitySourceId ? activityIds.get(item.activitySourceId) : null, item.name, isGoalIcon(item.materialIcon) ? item.materialIcon : "task_alt", config.repeatType, config.scheduleType, config.targetPerWeek, config.weekdaysMask, item.sortOrder, item.reminderEnabled ? 1 : 0, item.reminderTime ?? null, item.archived ? importedAt : null, "daylio", item.sourceId, item.sourceState ?? null);
      }),
    ];
