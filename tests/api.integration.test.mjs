...
test("Wrangler-backed API covers persistence, catalog, calendar, pagination, import, and export", async (t) => {
  ...
  const renamedActivity = await json(`/api/catalog/activity/${activity.body.activity.id}`, { method: "PATCH", body: JSON.stringify({ name: "Running", icon: "category" }) });
  assert.equal(renamedActivity.response.status, 200);
  assert.equal(renamedActivity.body.activity.name, "Running");
  assert.equal(renamedActivity.body.activity.icon, "fitness_center");
  const renamedActivityWithoutIcon = await json(`/api/catalog/activity/${activity.body.activity.id}`, { method: "PATCH", body: JSON.stringify({ name: "Still running" }) });
  assert.equal(renamedActivityWithoutIcon.response.status, 200);
  assert.equal(renamedActivityWithoutIcon.body.activity.icon, "fitness_center");
  ...
  const renamedGoal = await json(`/api/catalog/goal/${goal.body.goal.id}`, { method: "PATCH", body: JSON.stringify({ name: "  Test goal renamed  " }) });
  assert.equal(renamedGoal.response.status, 200);
  assert.equal(renamedGoal.body.goal.name, "Test goal renamed");
  const blankGoalName = await json(`/api/catalog/goal/${goal.body.goal.id}`, { method: "PATCH", body: JSON.stringify({ name: "   " }) });
  assert.equal(blankGoalName.response.status, 200);
  assert.equal(blankGoalName.body.goal.name, "Test goal renamed");
  ...
  const secondPage = await json("/api/entries?limit=1&offset=1");
  assert.equal(secondPage.response.status, 200);
  assert.equal(secondPage.body.entries.length, 1);
  assert.equal(secondPage.body.nextOffset, 2);
  assert.notEqual(secondPage.body.entries[0].logicalDate, page.body.entries[0].logicalDate);
  const fractionalPage = await json("/api/entries?limit=1.5&offset=1.5");
  assert.equal(fractionalPage.response.status, 200);
  assert.deepEqual(fractionalPage.body, secondPage.body);
